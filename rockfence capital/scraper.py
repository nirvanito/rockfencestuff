...
def scrape_atp(max_rank):
        ...
        for year, date_str in dates:
            ...
            while range_start < max_rank:
                ...
                for row in rows:
                    ...
                    if len(cols) >= 5:
                        try:
                            rank = cols[0].text_content(timeout=5000).strip()
                            raw_name = cols[1].text_content(timeout=5000).strip()
                            age = cols[2].text_content(timeout=5000).strip()
                        except Exception:
                            # skip rows with stale/detached DOM elements
                            continue

                        # make sure we dont grab the arrows
                        name = raw_name.split('\n')[-1].strip()

                        # handle tied ranks (e.g. "474T")
                        rank_clean = rank.rstrip("T")

                        # make sure rank is a number and within the rank bounds
                        if rank_clean.isdigit() and int(rank_clean) <= max_rank:
    ...
    csv_filename = f"2000-2025top{max_rank}rankedv2.csv"
