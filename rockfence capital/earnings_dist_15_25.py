...
# Load 2015-2025 data
df = pd.read_csv('rank_to_singles_inflation_adjusted.csv')
df = df.sort_values(['year', 'rank'], ascending=[True, True]).reset_index(drop=True)

# Filter to players who actually earned singles prize money
singles = df[df['singles_2025'] > 0]['singles_2025'].values
log_singles = np.log(singles)
top_earner_idx = df['singles_2025'].idxmax()
top_earner = df.loc[top_earner_idx, 'player name'] + f" ({df.loc[top_earner_idx, 'year']})"

print(f"Total observations in 2015-2025 data: {len(df)}")
print(f"Players with singles earnings > 0: {len(singles)}")
print(f"\n--- 2015-2025 Singles Earnings Summary (Inflation Adjusted) ---")
print(f"  Max   : ${singles.max():>12,.0f}  ({top_earner})")
...
for lo, hi, label in tiers:
    subset = df[(df['rank']>=lo) & (df['rank']<=hi)]['singles_2025']
...
ax_main.set_title('2015-2025 ATP Singles Earnings Distribution (Inflation Adj)', fontsize=14, fontweight='bold', pad=12)
ax_main.legend(fontsize=9, facecolor='#1a1a2e', edgecolor=GRID, labelcolor=TEXT)
style_ax(ax_main)
...
for lo, hi in tier_ranges:
    s = df[(df['rank']>=lo) & (df['rank']<=hi)]['singles_2025']
    s = s[s > 0]
...
ax_bar.set_title('Median vs Mean by Rank Tier (2015-2025)', fontsize=11, fontweight='bold')
...
ax_lorenz.set_title('Lorenz Curve (Earnings Inequality 2015-2025)', fontsize=11, fontweight='bold')
...
# ── Title ─────────────────────────────────────────────────────────────────
fig.text(0.5, 0.98, 'ATP 2015-2025 Singles Earnings Distribution (Inflation Adj)',
         ha='center', va='top', fontsize=15, fontweight='bold', color=TEXT)

plt.savefig('earnings_dist_15_25.png', dpi=180, bbox_inches='tight', facecolor=BG)
print("\nSaved earnings_dist_15_25.png")
